...
(function () {
  // Login removido - agora está em login.js
  ...
  function showSection(name) {
    const map = {
      ...
      cadastro: "cadastroSection",
      estoque: "estoqueSection",
      relatorios: "dashboardSection",
  ...
  function renderTable(filter = "") {
    ...
    const filtered = !filter
      ? items
      : items.filter((it) =>
          (it.modelo + it.categoria + it.ano + (it.desc || ""))
            .toLowerCase()
  ...
  function updateStats() {
    ...
    const avg = items.length
      ? items.reduce((s, i) => s + Number(i.preco), 0) / items.length
      : 0;
  ...
  function drawChart() {
    ...
    const counts = {};
    items.forEach(
      (i) => (counts[i.categoria] = (counts[i.categoria] || 0) + 1)
    );
    ...
    labels.forEach((lab, i) => {
      const x =
        padding +
        i * ((w - padding * 2) / Math.max(labels.length, 1)) +
        ((w - padding * 2) / Math.max(labels.length, 1) - barW) / 2;
  ...
  function openEditModal(idx) {
    ...
    body.innerHTML = `
      <div style="display:flex;gap:12px;flex-wrap:wrap">
        <div style="flex:1"><label>Modelo</label><input id="m_modelo" value="${
          it.modelo
        }"></div>
        <div style="flex:1"><label>Ano</label><input id="m_ano" type="number" value="${
          it.ano
        }"></div>
        <div style="flex:1"><label>Preço</label><input id="m_preco" type="number" value="${
          it.preco
        }"></div>
        <div style="flex:1"><label>Categoria</label><input id="m_cat" value="${
          it.categoria
        }"></div>
      </div>
      <div style="margin-top:8px"><label>Descrição</label><textarea id="m_desc" rows="3">${
        it.desc || ""
      }</textarea></div>
    `;
    const actions = document.getElementById("modalActions");
    actions.innerHTML =
      '<button class="btn ghost" id="modalCancel">Cancelar</button><button class="btn primary" id="modalSave">Salvar</button>';
      ...
      <div class="stock-item card" style="display:flex;gap:12px;align-items:center;margin-bottom:10px">
        ${
          it.foto
            ? '<img src="' +
              it.foto +
              '" style="height:68px;border-radius:8px">'
            : ""
        }
        <div style="flex:1"><strong>${
          it.modelo
        }</strong><div class="muted">Ano ${it.ano} • ${it.categoria}</div></div>
        <div style="text-align:right"><div style="font-weight:800">${formatMoney(
          it.preco
        )}</div><div style="margin-top:8px"><button class="btn edit" data-idx="${idx}">Editar</button> <button class="btn ghost danger del" data-idx="${idx}">Excluir</button></div></div>
      </div>`
      )
      .join("");
    attachRowEvents();
  ...
  document.getElementById("exportCsv")?.addEventListener("click", function () {
    const items = getVehicles();
    if (!items.length) {
      showToast("Nenhum veículo para exportar");
      return;
    }
    const rows = [["Modelo", "Ano", "Preço", "Categoria", "Descrição"]].concat(
      items.map((i) => [i.modelo, i.ano, i.preco, i.categoria, i.desc])
    );
    const csv = rows
  ...
  document.getElementById("clearData")?.addEventListener("click", function () {
    if (confirm("Apagar TODOS os veículos?")) {
  ...
  function showConfirm(title, message, onOk) {
    ...
    document.getElementById("modalActions").innerHTML =
      '<button class="btn ghost" id="modalCancel">Cancelar</button><button class="btn primary" id="modalOk">Confirmar</button>';
    showModal();
