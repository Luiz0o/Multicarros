  ...
  try {
    ...
    if (files && files.length) {
      for (let i = 0; i < files.length; i++) {
        ...
        const path = `veiculos/${created2.id}/${Date.now()}-${i}.${ext}`;
        const foto = yield uploadAndCreateFoto(Number(created2.id), f.buffer, path, f.mimetype, i + 1);
...
// src/routes/index.ts
var route = (app) => {
  app.route("/").get((req, res) => res.status(200).send({ message: "API Multi Carros" }));
  app.use(import_express6.default.json());
  app.use(veiculoRoutes_default);
  app.use(clienteRoutes_default);
  app.use(funcionarioRoutes_default);
  app.use(acessorioRoutes_default);
  app.use(usuarioRoutes_default);
};
