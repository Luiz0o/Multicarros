...
export const getAllUsuarios = async (): Promise<Usuario[]> => {
    const { data: usuarios, error } = await supabase
        .from('usuarios')
        .select('*');
        if (!usuarios) throw new HttError(Number(error.code), 'Erro ao buscar usuários');
...
export const getUsuarioById = async (id: string): Promise<Usuario | null> => {
    ...
    if (error) {
        ...
        throw new HttError(Number(error.code), `Erro ao buscar usuário: ${error.message}`); // Outro erro ocorreu
    }
    return usuarios as Usuario;
}   
...
export const createUsuario = async (novoUsuario: Usuario): Promise<Usuario> => {
    ...
    if (error) {
        ...
        // Erro genérico
        throw new HttError(Number(error.code), `Erro ao criar usuário: ${error.message}`);
    }
    
    return usuarios as Usuario;
}

export const atualizarUsuario = async (id: string, usuarioAtualizado: Partial<Usuario>) => {
    const { data: usuarios, error } = await supabase
        .from('usuarios')
        ...
        .select()
        .single();
    if (error) throw new HttError(Number(error.code), `Erro ao atualizar usuário: ${error.message}`);
    return usuarios as Usuario;
}   
...
export const deletarUsuario = async (id: string): Promise<boolean> => {
    const { error } = await supabase
        ...
        .delete()
        .eq('id', id);
    if (error) throw new HttError(Number(error.code), `Erro ao deletar usuário: ${error.message}`);
    return true;
}