...
export const getVeiculoById = async (placa: string) => {
    const veiculo = await veiculoRepository.getVeiculoById(placa);
    let response = null;
...
export const createVeiculo = async (novoVeiculo: any) => {
    try {
        // Validações básicas
        if (!novoVeiculo) {
        ...
        await veiculoRepository.createVeiculo(novoVeiculo);
